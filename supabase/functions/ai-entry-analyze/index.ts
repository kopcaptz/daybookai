...
serve(async (req) => {
  ...
  try {
    const { text, tags, language }: AnalyzeRequest = await req.json();
    ...
    const systemPrompt = language === "ru"
      ? `Ты — анализатор дневниковых записей. Анализируй текст и возвращай:
...
- Возвращай ТОЛЬКО валидный JSON`
      : `You are a diary entry analyzer. Analyze the text and return:
...
Return ONLY valid JSON:
{
  ...
  "semanticTags": ["tag1", "tag2", ...]
    ...
    console.log(`[${requestId}] Analyzing entry, textLen=${text.length}, tags=${tags.length}`);
    ...
    let parsed: { mood: number; confidence: number; semanticTags: string[] };
    try {
    ...
    console.log(`[${requestId}] Analysis complete: mood=${mood}, confidence=${confidence.toFixed(2)}, tags=${semanticTags.length}, duration=${durationMs}ms`);
