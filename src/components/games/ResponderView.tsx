...
import { GameRound, respondToSituation } from '@/lib/gameService';
...
export function ResponderView({
  ...
  const handleSubmit = async () => {
    const finalAnswer = showCustom ? 'custom' : selectedAnswer;
    if (!finalAnswer) return;

    setIsSubmitting(true);
    setError(null);

    const result = await respondToSituation(
      sessionId,
      finalAnswer,
      showCustom ? customAnswer : undefined
    );
  ...
  return (
    <div className="space-y-4">
      ...
      <Button
        ...
        disabled={
          isSubmitting ||
          (!selectedAnswer && !showCustom) ||
          (showCustom && !customAnswer.trim())
        }
        className="w-full"
      >
        {isSubmitting ? (
